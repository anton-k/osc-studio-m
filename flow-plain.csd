...
instr   LoadMsg
    SFile init ""     
    kChannel  init 0
    kType init $PLAY_LOOP

    kk init 0

nxtmsg:
    kk  OSClisten gihandle, "/load", "isi", kChannel, SFile, kType
if (kk == 0 || kChannel >= $SIZE || kChannel < 0) goto ex
    ...
    gkPlayTypes[kChannel] = kType
    gkFileTypes[kChannel] = kIsMp3
    kgoto nxtmsg
ex:
endin
...
nxtmsg:
    kk  OSClisten gihandle, "/stop-all", "i", kChannel
if (kk == 0) goto ex

    kChannel = 0 
    until (kChannel == $SIZE) do 
...
if (kk == 0 || kChannel >= $SIZE || kChannel < 0) goto ex
    ...
    if (gkPlayTypes[kChannel] > 0) then
