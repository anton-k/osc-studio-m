<CsoundSynthesizer>
<CsOptions>
-odac:nil  -d -+rtaudio=jack  -+jack_client=sampler 
</CsOptions>
<CsInstruments>
...
gkNextOutputs[] init $SIZE
gkNextRatios[]  init $SIZE
...
gkPlaybackType[]        init $MEMORY_SIZE
gkNextPlaybackType[]    init $MEMORY_SIZE
...
opcode RunDirectMono, aa, i
    iId xin
    iTab1 = giTab1s[iId]
    aSnd lposcil3 1, 1, 0, ftlen(iTab1), iTab1
    xout aSnd, aSnd    
endop 
...
opcode RunMono, aa, ia
    ...
    aSnd  mincer     aPointer, 1, 1, iTab1, giPhaseLock, ifftsize
    xout aSnd, aSnd
endop
...
opcode RunDirectStereo, aa, i
    ...
    aSnd1 lposcil3 1, 1, 0, ftlen(iTab1), iTab1
    aSnd2 lposcil3 1, 1, 0, ftlen(iTab2), iTab2
    xout aSnd1, aSnd2
...
opcode RunStereo, aa, ia
    ...
    aL  mincer     aPointer, 1, 1, iTab1, giPhaseLock, ifftsize   
    aR  mincer     aPointer, 1, 1, iTab2, giPhaseLock, ifftsize    
    xout aL, aR
endop
...
if (gkIsActive[kId] != 0) goto ex 
    printks "delete", 0    
    Sevt sprintfk {{i 45 0 0.01 %d}} , kId
    scoreline Sevt, 1    
    kgoto nxtmsg
ex:    
endin
...
if (gkTypes[kId] == 0) goto ex
    ...
    gkNextPlaybackType[kChannel] = $PLAYBACK_TYPE_FLEXIBLE
...
nxtmsg:
    kk  OSClisten gihandle, "/play_direct", "iii", kChannel, kId, kOut
...
if (gkTypes[kId] == 0) goto ex
    printks "play", 0
    gkNextIds[kChannel] = kId
    gkNextOutputs[kChannel] = kOut
    gkNextIsPlays[kChannel] = 1
    gkNextPlaybackType[kChannel] = $PLAYBACK_TYPE_DIRECT
    gkChanges[kChannel] = 1
    gkIsActive[kChannel] = gkIsActive[kChannel] + 1
    kgoto nxtmsg
ex: 
...
instr SetLoopRatioMsg 
    kChannel    init 0
    kLoopRatio  init 0    
    kk init 0
nxtmsg:
    kk  OSClisten gihandle, "/set_loop_ratio", "if", kChannel, kLoopRatio
if (kk == 0) goto ex 
if (kChannel < 0 || kChannel >= $SIZE) goto ex
    printks "set_loop_ratio", 0

    gkNextRatios[kChannel] = kLoopRatio
    kgoto nxtmsg
ex:    
...
instr SetOutputMsg 
    ...
    kk init 0
nxtmsg:
    kk  OSClisten gihandle, "/set_output", "if", kChannel, kOutput
if (kk == 0) goto ex 
if (kChannel < 0 || kChannel >= $SIZE) goto ex
    printks "set_output", 0

    gkNextOutputs[kChannel] = kOutput
    kgoto nxtmsg
ex:    
...
instr PlayLoop    
    ...
    if kLocalTick == 1 && gkChanges[kChannel] == 1 then
        ...
        elseif (gkNextIsPlays[kChannel] == 1 && (gkNextIds[kChannel] != gkIds[kChannel] || gkIsPlays[kChannel] == 0)) then
            ...
            turnoff2 kInstrId, 4, gkFadeTime
            event "i", kNextInstrId, 0, -1, gkNextIds[kChannel], iChannel, gkNextPlaybackType[kChannel]
        ...
        gkPlaybackType[iChannel] = gkNextPlaybackType[iChannel]
...
instr TheHeart
    ...
    kGain  port gkMasterGain, 0.05    
    kAmp   = kVol * gkLevel * kGain * giVolumeScale
...
instr Init
    ...
    while ki < $SIZE do
        ...
        gkVolumes[ki] = 1
        gkGains[ki] = 1
        gkPlaybackType[ki] = $PLAYBACK_TYPE_FLEXIBLE
        gkNextPlaybackType[ki] = $PLAYBACK_TYPE_FLEXIBLE
